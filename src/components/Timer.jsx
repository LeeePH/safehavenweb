...
import { addMonths, differenceInDays, differenceInHours, differenceInMinutes, differenceInMonths, differenceInSeconds } from 'date-fns';
...
function Timer() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      // Calculate full months first
      const months = Math.abs(differenceInMonths(now, START_DATE));

      // Then calculate remaining days after subtracting those months
      const dateAfterMonths = addMonths(START_DATE, months);
      const days = Math.abs(differenceInDays(now, dateAfterMonths));
    ...
    <div className="min-h-screen bg-black/20 flex flex-col items-center justify-center bg-cover bg-center relative text-white px-4">
      <div className="text-center z-10">
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-4 text-5xl sm:text-7xl font-bold mb-6">
          {['months', 'days', 'hours', 'minutes', 'seconds'].map((unit, index) => (
